...
test('POST /download rejects invalid URLs as JSON', async () => {
  ...
  await withServer(app, async (baseUrl) => {
    const response = await fetch(`${baseUrl}/download`, {
      body: JSON.stringify({ url: 'https://example.com/example/status/1234567890' }),
    ...
    assert.deepEqual(await response.json(), {
      code: 'invalid_url',
      message: 'Only x.com and twitter.com post URLs are supported.',
    })
  })
})
...
test('POST /download returns no_videos when extraction finds nothing', async () => {
  ...
  await withServer(app, async (baseUrl) => {
    const response = await fetch(`${baseUrl}/download`, {
      body: JSON.stringify({ url: 'https://x.com/example/status/1234567890' }),
      headers: { 'Content-Type': 'application/json' },
      method: 'POST',
    })
...
test('POST /download streams a ZIP for multiple videos', async () => {
  ...
  await withServer(app, async (baseUrl) => {
    const response = await fetch(`${baseUrl}/download`, {
      body: JSON.stringify({ url: 'https://twitter.com/example/status/1234567890' }),
      headers: { 'Content-Type': 'application/json' },
      method: 'POST',
    })
...
test('POST /download caps extracted videos before ZIP creation', async () => {
  const fetched = []
  ...
  await withServer(createApp({
    ...
    extractVideos: async () => [
      video({ index: 1, title: 'First Clip', url: 'https://cdn.example/first.mp4' }),
      video({ index: 2, title: 'Second Clip', url: 'https://cdn.example/second.mp4' }),
      video({ index: 3, title: 'Third Clip', url: 'https://cdn.example/third.mp4' }),
    ],
    fetchImpl: async (url) => {
      fetched.push(url)
