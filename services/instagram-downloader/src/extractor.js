...
export function filenameForVideo(video, { includeIndex = false } = {}) {
  const rawTitle = video.title || video.id || 'instagram-video'
  const cleanedTitle = sanitize(rawTitle).replace(/\s+/g, ' ').trim().slice(0, 80)
  const title = cleanedTitle || 'instagram-video'
...
export function zipFilename(videos) {
  ...
  const rawTitle = first.title || first.id || 'instagram-videos'
  const cleanedTitle = sanitize(rawTitle).replace(/\s+/g, ' ').trim().slice(0, 70)
  return `${cleanedTitle || 'instagram-videos'}-videos.zip`
}

export function collectVideoItems(info, { maxItems = 20, pageUrl = '' } = {}) {
  const videos = []
  const seenUrls = new Set()
  const rootTitle = info?.title || info?.fulltitle || info?.display_id || info?.id || 'instagram-video'
...
export async function extractInstagramVideos(pageUrl, { maxItems = 20, timeoutMs = 30000 } = {}) {
  let info
  ...
  try {
    ...
    throw new DownloaderError(
      'extract_failed',
      'Could not read public Instagram media for that URL.',
      502,
    )
  }
