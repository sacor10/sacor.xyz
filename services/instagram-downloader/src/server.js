...
import { extractInstagramVideos } from './extractor.js'
import { validateInstagramUrl } from './instagram.js'
...
function sendJsonError(res, error) {
  ...
  const message = error instanceof DownloaderError
    ? error.message
    : 'The Instagram downloader failed unexpectedly.'
...
export function createApp({
  config = readConfig(),
  extractVideos = extractInstagramVideos,
  fetchImpl = fetch,
} = {}) {
  const app = express()
  ...
  app.post('/download', async (req, res) => {
    try {
      const target = validateInstagramUrl(req.body?.url)
