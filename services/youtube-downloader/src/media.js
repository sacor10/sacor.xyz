  ...
  return new Transform({
    transform(chunk, _encoding, callback) {
      ...
      if (Number.isFinite(maxBytes) && total > maxBytes) {
        callback(new DownloaderError('download_too_large', 'The YouTube video is too large to download.', 413))
        return
      }
      callback(null, chunk)
    },
...
export async function fetchVideo(video, { fetchImpl = fetch, maxBytes, timeoutMs = 30000 } = {}) {
  ...
  try {
    const response = await fetchImpl(video.url, {
      headers: {
        Accept: '*/*',
        Referer: video.pageUrl || 'https://www.youtube.com/',
    ...
    if (!response?.ok || !response.body) {
      throw new DownloaderError(
        'download_failed',
        'Could not fetch one or more YouTube video files.',
        502,
      )
    }

    if (contentLengthTooLarge(response, maxBytes)) {
      throw new DownloaderError('download_too_large', 'The YouTube video is too large to download.', 413)
    }
