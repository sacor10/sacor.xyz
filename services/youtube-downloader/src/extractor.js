...
function hasAudio(candidate) {
  return candidate.acodec && candidate.acodec !== 'none'
}

function isMp4Candidate(candidate) {
  return ['mp4', 'm4v', 'mov'].includes(cleanExt(candidate.ext))
}
...
function pickVideoCandidate(item) {
  ...
  const preferred = candidates.filter((candidate) => hasAudio(candidate) && isMp4Candidate(candidate))
...
export function filenameForVideo(video, { includeIndex = false } = {}) {
  const rawTitle = video.title || video.id || 'youtube-video'
  const cleanedTitle = sanitize(rawTitle).replace(/\s+/g, ' ').trim().slice(0, 80)
  const title = cleanedTitle || 'youtube-video'
...
export function zipFilename(videos) {
  ...
  const rawTitle = first.title || first.id || 'youtube-videos'
  const cleanedTitle = sanitize(rawTitle).replace(/\s+/g, ' ').trim().slice(0, 70)
  return `${cleanedTitle || 'youtube-videos'}-videos.zip`
}

export function collectVideoItems(info, { maxItems = 20, pageUrl = '' } = {}) {
  const videos = []
  const seenUrls = new Set()
  const rootTitle = info?.title || info?.fulltitle || info?.display_id || info?.id || 'youtube-video'
...
export async function extractYouTubeVideos(pageUrl, { maxItems = 20, timeoutMs = 30000 } = {}) {
  let info
  ...
  try {
    ...
    throw new DownloaderError(
      'extract_failed',
      'Could not read public YouTube media for that URL.',
      502,
    )
  }
