...
import { extractYouTubeVideos } from './extractor.js'
...
function sendJsonError(res, error) {
  ...
  const message = error instanceof DownloaderError
    ? error.message
    : 'The YouTube downloader failed unexpectedly.'
...
export function createApp({
  config = readConfig(),
  extractVideos = extractYouTubeVideos,
  fetchImpl = fetch,
} = {}) {
  const app = express()
  ...
  app.post('/download', async (req, res) => {
    try {
      const target = validateYouTubeUrl(req.body?.url)
