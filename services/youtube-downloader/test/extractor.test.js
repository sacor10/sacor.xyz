...
test('collectVideoItems prefers audio-bearing MP4 over WebM when available', () => {
  const videos = collectVideoItems({
    entries: [
      {
        formats: [
          {
            ...
            ext: 'webm',
            height: 720,
            url: 'https://cdn.example/720.webm',
            vcodec: 'vp9',
          },
          {
            acodec: 'mp4a',
  ...
  assert.equal(videos.length, 1)
  assert.equal(videos[0].url, 'https://cdn.example/360.mp4')
})

test('collectVideoItems excludes audio-only formats', () => {
  const videos = collectVideoItems({
    entries: [
      {
        formats: [
          { acodec: 'opus', ext: 'webm', url: 'https://cdn.example/audio.webm', vcodec: 'none' },
          { acodec: 'mp4a', ext: 'mp4', height: 720, url: 'https://cdn.example/video.mp4', vcodec: 'h264' },
        ],
        id: 'clip',
        title: 'Clip',
      },
  ...
  assert.equal(videos.length, 1)
  assert.equal(videos[0].url, 'https://cdn.example/video.mp4')
})
