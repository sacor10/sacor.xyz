...
- **Tier C — role-specific sources**: published union wage scales, union-published teacher salary schedules (deliberately not the district's own site — a school district is a government body), Vivian Health nursing listings, carrier driver-pay pages, and an attempted Levels.fyi integration (see below). No keys needed. Registry: `src/data/payIndex/roleSources.ts`.
...
- **Vivian Health has no documented public API**, so that adapter targets a defensively-parsed generic response shape and is likely to need updating if the real shape differs — a mismatch shows up as a zero-rows alert, never fabricated data.
- **Levels.fyi's integration is unverified, including whether the endpoint exists at all.** Levels.fyi has no official public API. `levelsfyi.ts` targets `/companies/{slug}/salaries.md`, a URL pattern reported in third-party writeups as a crawler-facing markdown export — but this project's environment couldn't fetch it to confirm it resolves, is stable, or matches the assumed table shape, and its `robots.txt`/ToS haven't been checked for whether automated access is even permitted. If you rely on this adapter, verify the URL manually first; if it doesn't hold up, remove `levelsFyiAdapter` from `netlify/functions/_lib/payindex/adapters/registry.ts` rather than let it run against a source that may not exist. Its figures are excluded from the headline regardless (already-averaged, rule 7) — this only affects transparency-page context, not the number.
